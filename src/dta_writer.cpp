...
void DtaWriter::WriteBytes(const void *buf, size_t n) {
	const char *ptr = static_cast<const char *>(buf);
	size_t remaining = n;
	while (remaining > 0) {
		int64_t written = handle_->Write(const_cast<char *>(ptr), remaining);
		if (written <= 0) {
...
DtaWriter::DtaWriter(duckdb::FileSystem &fs, const std::string &path, const std::vector<DtaWriteColumn> &columns,
                     const std::string &dataset_label)
    : columns_(columns), dataset_label_(dataset_label), row_width_(0), map_content_pos_(0) {
	...
	handle_ = fs.OpenFile(path, duckdb::FileFlags::FILE_FLAGS_WRITE | duckdb::FileFlags::FILE_FLAGS_FILE_CREATE_NEW);
}
...
void DtaWriter::WriteMetadata() {
	...
	WriteU16(label_len);
	if (label_len > 0) {
		WriteBytes(dataset_label_.data(), label_len);
	}
	WriteTag("</label>");
	...
	offsets_[1] = handle_->SeekPosition();
	WriteTag("<map>");
	map_content_pos_ = handle_->SeekPosition();
	for (int i = 0; i < 14; i++) {
		WriteU64(0);
	}
	WriteTag("</map>");
	...
	// <variable_types>
	offsets_[2] = handle_->SeekPosition();
	...
	offsets_[3] = handle_->SeekPosition();
	WriteTag("<varnames>");
	for (auto &col : columns_) {
	...
	offsets_[4] = handle_->SeekPosition();
	WriteTag("<sortlist>");
	...
	offsets_[5] = handle_->SeekPosition();
	WriteTag("<formats>");
	for (auto &col : columns_) {
	...
	offsets_[6] = handle_->SeekPosition();
	...
	offsets_[7] = handle_->SeekPosition();
...
void DtaWriter::Finalize(uint64_t total_obs) {
	...
	// <value_labels>
	offsets_[11] = handle_->SeekPosition();
	...
	for (auto &vl : value_labels_) {
		...
		WriteBytes(txt_concat.data(), txtlen);
	...
	// </stata_dta>
	offsets_[12] = handle_->SeekPosition();
	WriteTag("</stata_dta>");
	offsets_[13] = handle_->SeekPosition();
